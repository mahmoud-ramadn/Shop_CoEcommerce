...
<template>
  <div class="container py-16 flex flex-col items-center border-b">
    ...
    <div class="flex gap-4 mt-4" v-if="!pending && !error && productByCatories?.length">
      <button
...
const totalProducts = computed(() => productByCatories?.length || 0);
...
const loadMoreItems = () => {
  ...
  const newItems = productByCatories?.slice(currentLength, currentLength + itemsPerLoad) || [];
...
const resetItems = () => {
  visibleProducts.value = productByCatories.slice(0, initialItemCount) || [];
  updateSwiper();
};
